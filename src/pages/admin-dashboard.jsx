...
const AddEditUserModal = ({
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center z-50 p-4 bg-opacity-50 dark:bg-opacity-70" id="modal-backdrop">
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg w-full max-w-md p-6">
        <div className="p-6">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-xl font-semibold text-gray-800 dark:text-gray-200">{isEditMode ? 'Edit Pengguna' : 'Tambah Pengguna Baru'}</h3>
            <button onClick={onClose} className="text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200" disabled={isLoading}><Icon classes="fas fa-times" /></button>
          </div>
          {error && <p className="text-red-500 text-sm mb-2 p-3 bg-red-100 dark:bg-red-800/30 dark:text-red-300 rounded-md">{error}</p>}
          {successMessage && <p className="text-green-500 text-sm mb-2 p-3 bg-green-100 dark:bg-green-800/30 dark:text-green-300 rounded-md">{successMessage}</p>}
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label htmlFor="user-type-modal" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Tipe Pengguna</label>
              <select
                id="user-type-modal"
                value={userType}
                onChange={(e) => setUserType(e.target.value)}
                disabled={isLoading || isEditMode}
                className="mt-1 block w-full input-field-dark">
            ...
            <div>
              <label htmlFor="name-modal" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Nama Lengkap</label>
              <input type="text" id="name-modal" value={name} onChange={(e) => setName(e.target.value)} required disabled={isLoading} className="mt-1 block w-full input-field-dark" />
            </div>
            <div>
              <label htmlFor="email-modal" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Email</label>
              <input type="email" id="email-modal" value={email} onChange={(e) => setEmail(e.target.value)} required disabled={isLoading} className="mt-1 block w-full input-field-dark" />
            </div>

            {userType === 'student' && (
              <div>
                <label htmlFor="student-class-level-modal" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Kelas Siswa</label>
                <select
                 ...
                 {classesList.length === 0 && !isLoadingDropdownData && <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Tidak ada pilihan kelas tersedia.</p>}
              </div>
            )}

            {userType === 'teacher' && (
              <div>
                <label htmlFor="teacher-main-subject-modal" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Mapel Utama Guru <span className="text-xs text-gray-500 dark:text-gray-400">(Opsional)</span></label>
                <select
                ...
                {coursesList.length === 0 && !isLoadingDropdownData && <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Tidak ada pilihan mata pelajaran tersedia.</p>}
              </div>
            )}

            <div>
              <label htmlFor="password-modal" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Kata Sandi {isEditMode ? '(Kosongkan jika tidak diubah)' : ''}</label>
              <input type="password" id="password-modal" value={password} onChange={(e) => setPassword(e.target.value)} required={!isEditMode} disabled={isLoading} placeholder={isEditMode ? "Tidak diubah" : "Min. 6 karakter"} className="mt-1 block w-full input-field-dark" />
            </div>
...
const UserTable = ({ userType, onEdit, onDelete, needsRefresh, searchTerm, sortOrder, classFilter }) => {
  ...
  if (error) return <div className="text-center py-10 text-red-500 bg-red-50 dark:bg-red-800/20 dark:text-red-400 p-4 rounded-md">{error}</div>;
  if (processedUsers.length === 0) return <div className="text-center py-10 text-gray-500 dark:text-gray-400">Tidak ada data yang cocok dengan kriteria.</div>;

  return (
    <div className="overflow-x-auto">
      <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
        <thead className="bg-gray-50 dark:bg-gray-700/50"><tr>{tableHeaders.map(h => <th key={h} scope="col" className="th-cell-dark">{h}</th>)}</tr></thead>
        <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
          {processedUsers.map((user) => (
            <tr key={user.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
              ...
              <td className="td-cell-dark text-gray-500 dark:text-gray-400">{user.email}</td>
              <td className="td-cell-dark text-gray-500 dark:text-gray-400">
              ...
              <td className="td-cell-dark text-sm font-medium">
                <button onClick={() => onEdit(user)} className="text-indigo-600 hover:text-indigo-800 dark:text-indigo-400 dark:hover:text-indigo-300 font-medium py-1 px-2 rounded-md hover:bg-indigo-50 dark:hover:bg-indigo-900/50 transition-colors duration-150 mr-2">Edit</button>
                <button onClick={() => onDelete(user.id, userType)} className="text-red-600 hover:text-red-800 dark:text-red-500 dark:hover:text-red-400 font-medium py-1 px-2 rounded-md hover:bg-red-50 dark:hover:bg-red-900/50 transition-colors duration-150">Delete</button>
              </td>
            </tr>
          ))}
        </tbody>
...
const UserManagementView = () => {
  ...
  return (
    <div>
      ...
      {dropdownError && <p className="text-red-500 text-sm mb-2 p-3 bg-red-100 dark:bg-red-800/20 dark:text-red-400 rounded-md text-center">{dropdownError}</p>}
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 mb-6">
        <div className="flex border-b border-gray-200 dark:border-gray-700 mb-6">
          <button onClick={() => setActiveTab('teachers')} className={`py-2 px-4 font-medium hover:text-blue-600 dark:hover:text-blue-400 border-b-2 ${activeTab === 'teachers' ? 'text-blue-600 border-blue-600 dark:text-blue-400 dark:border-blue-500' : 'text-gray-500 border-transparent dark:text-gray-400'}`}>Guru</button>
          <button onClick={() => setActiveTab('students')} className={`py-2 px-4 font-medium hover:text-blue-600 dark:hover:text-blue-400 border-b-2 ${activeTab === 'students' ? 'text-blue-600 border-blue-600 dark:text-blue-400 dark:border-blue-500' : 'text-gray-500 border-transparent dark:text-gray-400'}`}>Siswa</button>
        </div>
            ...
            <div className="flex flex-wrap items-center gap-4">
                <div>
                    <select
                        ...
                        className="py-2 pl-3 pr-8 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white dark:bg-gray-700 dark:border-gray-600 dark:text-white"
                    >
...
const AddEditCourseModal = ({ isOpen, onClose, onCourseSaved, editingCourse }) => {
    ...
    return (
        <div className="fixed inset-0 flex items-center justify-center z-50 p-4 bg-black bg-opacity-50 dark:bg-opacity-70" id="modal-backdrop">
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg w-full max-w-md p-6">
                <div className="flex justify-between items-center mb-4">
                    <h3 className="text-xl font-semibold dark:text-gray-200">{isEditMode ? 'Edit Mata Pelajaran' : 'Tambah Mata Pelajaran'}</h3>
                    <button onClick={onClose} disabled={isLoading}><Icon classes="fas fa-times text-gray-600 hover:text-gray-800 dark:text-gray-400 dark:hover:text-gray-200" /></button>
                </div>
                {error && <p className="text-red-500 text-sm mb-2 p-3 bg-red-100 dark:bg-red-800/30 dark:text-red-300 rounded-md">{error}</p>}
                {successMessage && <p className="text-green-500 text-sm mb-2 p-3 bg-green-100 dark:bg-green-800/30 dark:text-green-300 rounded-md">{successMessage}</p>}
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div><label htmlFor="courseCode" className="block text-sm font-medium dark:text-gray-300">Kode</label><input type="text" id="courseCode" value={courseCode} onChange={e => setCourseCode(e.target.value)} required disabled={isLoading} className="mt-1 w-full input-field-dark" /></div>
                    <div><label htmlFor="courseName" className="block text-sm font-medium dark:text-gray-300">Nama Mata Pelajaran</label><input type="text" id="courseName" value={courseName} onChange={e => setCourseName(e.target.value)} required disabled={isLoading} className="mt-1 w-full input-field-dark" /></div>
                    <div><label htmlFor="description" className="block text-sm font-medium dark:text-gray-300">Deskripsi</label><textarea id="description" value={description} onChange={e => setDescription(e.target.value)} disabled={isLoading} className="mt-1 w-full input-field-dark" rows="3"></textarea></div>
...
const CoursesView = () => {
    ...
    return (
        <div>
            ...
            {courses.length === 0 ? (
                <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 text-gray-500 dark:text-gray-400 text-center py-5">
                    ...
                    <div className="overflow-x-auto p-6">
                        <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            <thead className="bg-gray-50 dark:bg-gray-700/50">
                                <tr>
                                    <th className="th-cell-dark">Kode</th>
                                    <th className="th-cell-dark">Nama Mata Pelajaran</th>
                                    <th className="th-cell-dark">Deskripsi</th>
                                    <th className="th-cell-dark">Aksi</th>
                                </tr>
                            </thead>
                            <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                {courses.map(course => (
                                    <tr key={course.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                        <td className="td-cell-dark">{course.course_code}</td>
                                        <td className="td-cell-dark">
                                            <div className="text-sm font-medium text-gray-900 dark:text-white">{course.course_name}</div>
                                        </td>
                                        <td className="td-cell-dark text-gray-500 dark:text-gray-400 truncate max-w-xs">{course.description || '-'}</td>
                                        <td className="td-cell-dark text-sm font-medium">
                                            <button onClick={() => handleEditCourse(course)} className="text-indigo-600 hover:text-indigo-800 dark:text-indigo-400 dark:hover:text-indigo-300 font-medium py-1 px-2 rounded-md hover:bg-indigo-50 dark:hover:bg-indigo-900/50 transition-colors duration-150 mr-2">Edit</button>
                                            <button onClick={() => handleDeleteCourse(course.id)} className="text-red-600 hover:text-red-800 dark:text-red-500 dark:hover:text-red-400 font-medium py-1 px-2 rounded-md hover:bg-red-50 dark:hover:bg-red-900/50 transition-colors duration-150">Delete</button>
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
...
const AddEditClassModal = ({ isOpen, onClose, onClassSaved, editingClass, teachersList }) => {
  ...
  return (
        <div className="fixed inset-0 flex items-center justify-center z-50 p-4 bg-black bg-opacity-50 dark:bg-opacity-70" id="modal-backdrop">
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg w-full max-w-md p-6">
                <div className="flex justify-between items-center mb-6">
                    <h3 className="text-xl font-semibold text-gray-800 dark:text-gray-200">{isEditMode ? 'Edit Kelas' : 'Tambah Kelas Baru'}</h3>
                    <button onClick={onClose} className="text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200" disabled={isLoading}><Icon classes="fas fa-times" /></button>
                </div>
                {error && <p className="text-red-500 text-sm mb-3 p-3 bg-red-100 dark:bg-red-800/30 dark:text-red-300 rounded-md">{error}</p>}
                {successMessage && <p className="text-green-500 text-sm mb-3 p-3 bg-green-100 dark:bg-green-800/30 dark:text-green-300 rounded-md">{successMessage}</p>}
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div>
                        <label htmlFor="class-name-modal" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Nama Kelas</label>
                        <input
                            type="text"
                            id="class-name-modal"
                            value={className}
                    ...
                    <div>
                        <label htmlFor="homeroom-teacher-modal" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Wali Kelas (Opsional)</label>
                        <select
                        ...
                        {teachersList.length === 0 && <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Tidak ada data guru tersedia.</p>}
                    </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg w-full max-w-2xl p-6">
                <div className="flex justify-between items-center mb-4 border-b dark:border-gray-700 pb-3">
                    <h3 className="text-xl font-semibold text-gray-800 dark:text-gray-200">
                        Daftar Siswa Kelas: <span className="text-blue-600 dark:text-blue-400">{classItem?.class_name}</span>
                    </h3>
                    <button onClick={onClose} className="text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200"><Icon classes="fas fa-times" /></button>
                </div>
                    ...
                    {isLoading ? (
                        ...
                        <div className="overflow-x-auto">
                          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                              <thead className="bg-gray-50 dark:bg-gray-700/50">
                                  <tr>
                                      <th className="th-cell-dark w-16">No.</th>
                                      <th className="th-cell-dark">Nama Siswa</th>
                                      <th className="th-cell-dark">Email</th>
                                  </tr>
                              </thead>
                              <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                  {students.map((student, index) => (
                                      <tr key={student.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                          <td className="td-cell-dark text-center">{index + 1}</td>
                                          <td className="td-cell-dark">{student.name}</td>
                                          <td className="td-cell-dark">{student.email}</td>
                                      </tr>
                                  ))}
                              </tbody>
                          </table>
                        ...
                        <p className="text-gray-500 dark:text-gray-400 text-center py-10">Tidak ada data siswa di kelas ini.</p>
                    )}
                </div>
...
const SettingsView = () => {
    ...
    return (
        <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
            <h3 className="text-xl font-semibold text-gray-800 dark:text-gray-200 mb-4">Pengaturan Umum</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-4">
                    <h4 className="font-semibold text-lg mb-2 dark:text-gray-200">Periode Semester</h4>
                    <div>
                        <label htmlFor="school_year_start_date" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Tanggal Mulai Semester</label>
                        <input type="date" id="school_year_start_date" name="school_year_start_date" value={settings.school_year_start_date || ''} onChange={handleInputChange} className="mt-1 block w-full input-field-dark"/>
                    </div>
                    <div>
                        <label htmlFor="school_year_end_date" className="block text-sm font-medium text-gray-700 dark:text-gray-300">Tanggal Selesai Semester</label>
                        <input type="date" id="school_year_end_date" name="school_year_end_date" value={settings.school_year_end_date || ''} onChange={handleInputChange} className="mt-1 block w-full input-field-dark"/>
                    </div>
                </div>

                <div className="space-y-4">
                     <h4 className="font-semibold text-lg mb-2 dark:text-gray-200">Area Sekolah (Geofence)</h4>
                     <p className="text-sm text-gray-500 dark:text-gray-400">Klik pada peta untuk menambahkan titik-titik yang membentuk area sekolah. Minimal 3 titik untuk membentuk sebuah area.</p>
                    <div className="h-80 w-full rounded-lg overflow-hidden border dark:border-gray-600">
                    ...
                    <button onClick={clearPolygon} className="btn-secondary-dark">
...
const ClassesView = () => {
  ...
  const renderClassCards = (classArray) => {
    if (classArray.length === 0) {
        return <p className="text-gray-500 dark:text-gray-400 col-span-full text-center py-3">Tidak ada kelas dalam kategori ini.</p>;
    }
    return classArray.map((classItem) => (
        <div key={classItem.id} className="bg-white dark:bg-gray-800 rounded-xl shadow-lg border border-gray-200 dark:border-gray-700 overflow-hidden flex flex-col">
            <div className="p-5 flex-grow">
            <h3 className="text-xl font-semibold text-gray-800 dark:text-gray-200 mb-2 truncate" title={classItem.class_name}>
                <Icon classes="fas fa-chalkboard mr-2 text-blue-500 dark:text-blue-400" />
                {classItem.class_name}
            </h3>
            <div className="text-sm text-gray-600 dark:text-gray-400 mb-1">
                <Icon classes="fas fa-user-tie mr-2 text-gray-400 dark:text-gray-500" />
                Wali Kelas: <span className="font-medium text-gray-700 dark:text-gray-300">{classItem.homeroom_teacher_name || <span className="italic">Belum ada</span>}</span>
            </div>
            <div className="text-sm text-gray-600 dark:text-gray-400">
                <Icon classes="fas fa-users mr-2 text-gray-400 dark:text-gray-500" />
                Jumlah Siswa: <span className="font-medium text-gray-700 dark:text-gray-300">{classItem.student_count}</span>
            </div>
            </div>
            <div className="bg-gray-50 dark:bg-gray-700/50 p-3 border-t border-gray-200 dark:border-gray-700 flex justify-end items-center space-x-2">
            <button
                onClick={() => handleViewDetails(classItem)}
                className="text-sm text-green-600 hover:text-green-800 dark:text-green-400 dark:hover:text-green-300 font-medium py-1 px-3 rounded-md hover:bg-green-50 dark:hover:bg-green-900/40 transition-colors duration-150 flex items-center"
                disabled={isLoading}
            ...
            <button
                ...
                className="text-sm text-indigo-600 hover:text-indigo-800 dark:text-indigo-400 dark:hover:text-indigo-300 font-medium py-1 px-3 rounded-md hover:bg-indigo-50 dark:hover:bg-indigo-900/40 transition-colors duration-150 flex items-center"
                disabled={isLoading}
                title="Edit Kelas"
            >
            ...
            <button
                ...
                className="text-sm text-red-600 hover:text-red-800 dark:text-red-500 dark:hover:text-red-400 font-medium py-1 px-3 rounded-md hover:bg-red-50 dark:hover:bg-red-900/40 transition-colors duration-150 flex items-center"
                disabled={isLoading}
                title="Hapus Kelas"
            >
  ...
  return (
    <div className="p-1">
      ...
      {isLoading && classes.length > 0 && <p className="text-center text-gray-500 dark:text-gray-400 my-4 px-6 md:px-0">Memperbarui data...</p>}

      {(classes.length === 0 && !isLoading) ? (
        <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
             <p className="text-gray-500 dark:text-gray-400 text-center py-5">Belum ada data kelas.</p>
        </div>
      ) : (
        <div className="space-y-8 px-6 pb-6 md:px-0 md:pb-0">
            <div>
                <h3 className="text-2xl font-semibold text-gray-700 dark:text-gray-200 mb-4 border-b-2 border-blue-500 pb-2 flex items-center">
            ...
            <div>
                <h3 className="text-2xl font-semibold text-gray-700 dark:text-gray-200 mb-4 border-b-2 border-green-500 pb-2 flex items-center">
            ...
            <div>
                <h3 className="text-2xl font-semibold text-gray-700 dark:text-gray-200 mb-4 border-b-2 border-purple-500 pb-2 flex items-center">
            ...
            {otherClasses.length > 0 && (
                <div>
                    <h3 className="text-2xl font-semibold text-gray-700 dark:text-gray-200 mb-4 border-b-2 border-gray-400 pb-2 flex items-center">
    ...
    return (
        <div className="fixed inset-0 flex items-center justify-center z-50 p-4 bg-black bg-opacity-50 dark:bg-opacity-70" id="modal-backdrop">
            <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg w-full max-w-lg p-6">
                <div className="flex justify-between items-center mb-6"><h3 className="text-xl font-semibold dark:text-gray-200">{isEditMode ? 'Edit Jadwal' : 'Tambah Jadwal Baru'}</h3><button onClick={onClose} disabled={isLoading}><Icon classes="fas fa-times text-gray-600 hover:text-gray-800 dark:text-gray-400 dark:hover:text-gray-200" /></button></div>
                {error && <p className="text-red-500 text-sm mb-4 p-3 bg-red-100 dark:bg-red-800/30 dark:text-red-300 rounded-md">{error}</p>}
                {successMessage && <p className="text-green-500 text-sm mb-4 p-3 bg-green-100 dark:bg-green-800/30 dark:text-green-300 rounded-md">{successMessage}</p>}
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div><label htmlFor="courseIdSchedule" className="block text-sm font-medium dark:text-gray-300">Mata Pelajaran</label><select id="courseIdSchedule" value={courseId} onChange={e => setCourseId(e.target.value)} required disabled={isLoading || isLoadingDropdown || courses.length === 0} className="mt-1 w-full input-field-dark">{courses.length === 0 && !isLoadingDropdown ? <option>Tidak ada mapel</option> : <option value="">Pilih Mapel</option>}{courses.map(c => <option key={c.id} value={c.id}>{c.course_name} ({c.course_code})</option>)}</select></div>
                        <div><label htmlFor="teacherIdSchedule" className="block text-sm font-medium dark:text-gray-300">Guru Pengampu</label><select id="teacherIdSchedule" value={teacherId} onChange={e => setTeacherId(e.target.value)} required disabled={isLoading || isLoadingDropdown || teachers.length === 0} className="mt-1 w-full input-field-dark">{teachers.length === 0 && !isLoadingDropdown ? <option>Tidak ada guru</option> : <option value="">Pilih Guru</option>}{teachers.map(t => <option key={t.id} value={t.id}>{t.name}</option>)}</select></div>
                    </div>
                    <div>
                        <label htmlFor="studentClassLevelSchedule" className="block text-sm font-medium dark:text-gray-300">Untuk Kelas</label>
                        <select
                            ...
                            className="mt-1 w-full input-field-dark">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div><label htmlFor="dayOfWeekSchedule" className="block text-sm font-medium dark:text-gray-300">Hari</label><select id="dayOfWeekSchedule" value={dayOfWeek} onChange={e => setDayOfWeek(e.target.value)} required disabled={isLoading} className="mt-1 w-full input-field-dark">{days.map(d => <option key={d} value={d}>{d}</option>)}</select></div>
                        <div><label htmlFor="startTimeSchedule" className="block text-sm font-medium dark:text-gray-300">Waktu Mulai</label><input type="time" id="startTimeSchedule" value={startTime} onChange={e => setStartTime(e.target.value)} required disabled={isLoading} className="mt-1 w-full input-field-dark" /></div>
                        <div><label htmlFor="endTimeSchedule" className="block text-sm font-medium dark:text-gray-300">Waktu Selesai</label><input type="time" id="endTimeSchedule" value={endTime} onChange={e => setEndTime(e.target.value)} required disabled={isLoading} className="mt-1 w-full input-field-dark" /></div>
                    </div>
                    <div><label htmlFor="roomNumberSchedule" className="block text-sm font-medium dark:text-gray-300">Nomor Ruangan <span className="text-xs dark:text-gray-400">(Opsional)</span></label><input type="text" id="roomNumberSchedule" value={roomNumber} onChange={e => setRoomNumber(e.target.value)} disabled={isLoading} className="mt-1 w-full input-field-dark" /></div>
...
const SchedulesView = () => {
    ...
    return (
        <div>
            ...
            <div className="mb-6">
                <h3 className="text-xl font-semibold text-gray-700 dark:text-gray-200 mb-3">Pilih Kelas:</h3>
                {isLoading && uniqueClasses.length === 0 && <p className="text-gray-500 dark:text-gray-400">Memuat daftar kelas...</p>}
                        ...
                        <button
                            ...
                            className={`p-4 rounded-lg shadow-md text-center font-medium transition-all duration-200 ease-in-out
                                        ${selectedClass === className
                                            ...
                                            : 'bg-white text-gray-800 hover:bg-gray-50 hover:shadow-lg dark:bg-gray-800 dark:text-gray-200 dark:hover:bg-gray-700'}`}
                        >
            ...
            {selectedClass && (
                <>
                    <h3 className="text-lg font-semibold mb-4 text-gray-700 dark:text-gray-200">Detail Jadwal untuk Kelas: <span className="text-blue-600 dark:text-blue-400">{selectedClass}</span></h3>
                    {isLoading && filteredSchedules.length === 0 && <p className="text-center text-gray-500 dark:text-gray-400 py-4">Memuat jadwal kelas {selectedClass}...</p>}
                        ...
                        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm overflow-hidden">
                            <div className="overflow-x-auto p-6">
                                <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                    <thead className="bg-gray-50 dark:bg-gray-700/50">
                                        <tr>
                                            <th className="th-cell-dark">Hari</th>
                                            <th className="th-cell-dark">Waktu</th>
                                            <th className="th-cell-dark">Mata Pelajaran (Kode)</th>
                                            <th className="th-cell-dark">Guru</th>
                                            <th className="th-cell-dark">Ruangan</th>
                                            <th className="th-cell-dark">Aksi</th>
                                        </tr>
                                    </thead>
                                    <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                        {filteredSchedules.map(sch => (
                                            <tr key={sch.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                                                <td className="td-cell-dark">{sch.day_of_week}</td>
                                                <td className="td-cell-dark">{sch.start_time_formatted} - {sch.end_time_formatted}</td>
                                                <td className="td-cell-dark">
                                                    <div className="text-sm font-medium text-gray-900 dark:text-white">{sch.course_name} ({sch.course_code})</div>
                                                </td>
                                                <td className="td-cell-dark">
                                                    <div className="text-sm font-medium text-gray-900 dark:text-white">{sch.teacher_name}</div>
                                                </td>
                                                <td className="td-cell-dark text-gray-500 dark:text-gray-400">{sch.room_number || '-'}</td>
                                                <td className="td-cell-dark text-sm font-medium">
                                                    <button onClick={() => handleEditSchedule(sch)} className="text-indigo-600 hover:text-indigo-800 dark:text-indigo-400 dark:hover:text-indigo-300 font-medium py-1 px-2 rounded-md hover:bg-indigo-50 dark:hover:bg-indigo-900/50 transition-colors duration-150 mr-2">Edit</button>
                                                    <button onClick={() => handleDeleteSchedule(sch.id)} className="text-red-600 hover:text-red-800 dark:text-red-500 dark:hover:text-red-400 font-medium py-1 px-2 rounded-md hover:bg-red-50 dark:hover:bg-red-900/50 transition-colors duration-150">Delete</button>
                                                </td>
                                            </tr>
                                        ))}
                                    </tbody>
...
const OnlineUsersView = () => {
  const [onlineUsers, setOnlineUsers] = useState({ teachers: [], students: [] });
  const [isLoading, setIsLoading] = useState(true);
  const [error, setError] = useState('');
    ...
    const fetchOnlineUsers = async () => {
      ...
      try {
        const [teachersRes, studentsRes] = await Promise.all([
          fetch(`${API_BASE_URL}/get_users.php?type=teachers`),
          fetch(`${API_BASE_URL}/get_users.php?type=students`)
        ]);
        ...
        if (teachersData.success && studentsData.success) {
          setOnlineUsers({
            teachers: (teachersData.data || []).filter(user => user.status === 'active'),
            students: (studentsData.data || []).filter(user => user.status === 'active')
          });
        } else {
  ...
  const renderUserList = (users, title) => (
    <div>
      <h3 className="font-semibold text-gray-700 dark:text-gray-300 mb-2">{title} ({users.length})</h3>
      {users.length > 0 ? (
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400">Tidak ada {title.toLowerCase()} yang online.</p>
      )}
    </div>
  );

  if (isLoading) {
    return <div className="text-center py-10"><Icon classes="fas fa-spinner fa-spin text-xl" /></div>;
  }
  if (error) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      {renderUserList(onlineUsers.teachers, "Guru Online")}
      {renderUserList(onlineUsers.students, "Siswa Online")}
    </div>
  );
};
//Dashboard Utama
const DashboardView = ({ onNavigate }) => {
  const [stats, setStats] = useState({ teacher_count: 0, student_count: 0, class_count: 0, course_count: 0 });
  ...
  const statsData = [
    ...
    { title: 'Total Kelas', value: isLoadingStats ? <Icon classes="fas fa-spinner fa-spin"/> : (stats.class_count || 0), icon: 'fa-school', color: 'purple' },
    { title: 'Total Mapel', value: isLoadingStats ? <Icon classes="fas fa-spinner fa-spin"/> : (stats.course_count || 0), icon: 'fa-book', color: 'yellow' },
  ];

  const quickActions = [
    { label: 'Tambah Pengguna', icon: 'fa-user-plus', color: 'blue', view: 'users' },
    { label: 'Tambah Mapel', icon: 'fa-book', color: 'green', view: 'courses'},
    { label: 'Tambah Kelas', icon: 'fa-school', color: 'orange', view: 'classes' },
    { label: 'Buat Jadwal', icon: 'fa-calendar-plus', color: 'purple', view: 'schedules'},
  ];

  const colorClasses = {
    blue: { bg: 'bg-blue-50 dark:bg-blue-900/30', text: 'text-blue-600 dark:text-blue-400', iconBg: 'bg-blue-100 dark:bg-blue-900/50', iconText: 'text-blue-800 dark:text-blue-300', buttonBg: 'bg-blue-100 dark:bg-blue-900/50', buttonHoverBg: 'hover:bg-blue-200 dark:hover:bg-blue-900/70', buttonText: 'text-blue-800 dark:text-blue-200' },
    green: { bg: 'bg-green-50 dark:bg-green-900/30', text: 'text-green-600 dark:text-green-400', iconBg: 'bg-green-100 dark:bg-green-900/50', iconText: 'text-green-800 dark:text-green-300', buttonBg: 'bg-green-100 dark:bg-green-900/50', buttonHoverBg: 'hover:bg-green-200 dark:hover:bg-green-900/70', buttonText: 'text-green-800 dark:text-green-200' },
    purple: { bg: 'bg-purple-50 dark:bg-purple-900/30', text: 'text-purple-600 dark:text-purple-400', iconBg: 'bg-purple-100 dark:bg-purple-900/50', iconText: 'text-purple-800 dark:text-purple-300', buttonBg: 'bg-purple-100 dark:bg-purple-900/50', buttonHoverBg: 'hover:bg-purple-200 dark:hover:bg-purple-900/70', buttonText: 'text-purple-800 dark:text-purple-200' },
    yellow: { bg: 'bg-yellow-50 dark:bg-yellow-900/30', text: 'text-yellow-600 dark:text-yellow-400', iconBg: 'bg-yellow-100 dark:bg-yellow-900/50', iconText: 'text-yellow-800 dark:text-yellow-300', buttonBg: 'bg-yellow-100 dark:bg-yellow-900/50', buttonHoverBg: 'hover:bg-yellow-200 dark:hover:bg-yellow-900/70', buttonText: 'text-yellow-800 dark:text-yellow-200' },
    orange: { bg: 'bg-orange-50 dark:bg-orange-900/30', text: 'text-orange-600 dark:text-orange-400', iconBg: 'bg-orange-100 dark:bg-orange-900/50', iconText: 'text-orange-800 dark:text-orange-300', buttonBg: 'bg-orange-100 dark:bg-orange-900/50', buttonHoverBg: 'hover:bg-orange-200 dark:hover:bg-orange-900/70', buttonText: 'text-orange-800 dark:text-orange-200' },
  };
  ...
  return (
    <div>
      {statsError && <div className="mb-4 p-3 bg-red-100 text-red-700 dark:bg-red-800/30 dark:text-red-300 rounded-md">{statsError}</div>}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        {statsData.map(stat => (
          <div key={stat.title} className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
            <div className="flex items-center justify-between">
              <div><p className="text-gray-500 dark:text-gray-400">{stat.title}</p><h3 className="text-2xl font-bold mt-1 text-gray-800 dark:text-gray-100">{stat.value}</h3></div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
          <h2 className="text-xl font-semibold text-gray-800 dark:text-gray-200 mb-4">Pengguna Online</h2>
          <OnlineUsersView />
        </div>
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 p-6">
          <h2 className="text-xl font-semibold text-gray-800 dark:text-gray-200 mb-4">Aksi Cepat</h2>
...
const AdminSidebar = ({ onNavigate, activeView, onLogout, isSidebarOpen, user }) => {
  ...
  return (
    <div className={`sidebar bg-white text-gray-800 dark:bg-gray-800 dark:text-gray-200 w-64 min-h-screen shadow-lg transition-transform duration-300 ease-in-out ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 md:static fixed md:z-40`}>
      <div className="p-4">
        <div className="flex items-center space-x-3 mb-6">
          ...
          />
          <div><p className="font-medium text-gray-800 dark:text-white">{user.name || "Admin"}</p><p className="text-sm text-gray-500 dark:text-gray-400">{user.role || "Administrator"}</p></div>
        </div>
        <nav className="space-y-2">
          {navItemsAdmin.map(item => (
            <a key={item.name} href="#" onClick={(e) => { e.preventDefault(); onNavigate(item.view); }} className={`nav-item flex items-center space-x-3 p-3 rounded-lg transition-colors ${activeView === item.view ? 'bg-blue-100 dark:bg-blue-900/50 text-blue-600 dark:text-blue-300' : 'hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300'}`}>
      ...
      <div className="absolute bottom-0 w-full p-4 border-t border-gray-200 dark:border-gray-700">
        <button onClick={onLogout} className="flex items-center space-x-3 p-3 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300 w-full"><Icon classes="fas fa-sign-out-alt w-5 text-center" /><span className="sidebar-text">Logout</span></button>
      </div>
    </div>
  );
};
...
const AdminMainContent = ({ activeView, onToggleSidebar, onNavigate }) => {
  ...
  return (
    <div className="main-content flex-1 p-4 sm:p-8 bg-gray-100 dark:bg-gray-900 min-h-screen overflow-y-auto">
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-200">{title}</h1>
        <button onClick={onToggleSidebar} className="p-2 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-700 md:hidden"><Icon classes="fas fa-bars text-gray-700 dark:text-gray-300" /></button>
      </div>
...
export default function AdminDashboard({ onLogout, user }) {
  ...
  return (
    <div className="flex min-h-screen font-sans antialiased text-gray-900 dark:text-gray-200 bg-gray-100 dark:bg-gray-900">
      <AdminSidebar
...
// Helper CSS classes to be used in className attributes for dark mode consistency
/*
  In your global CSS file (e.g., index.css), you can define these helper classes.
  This is just a suggestion for centralization. The code above applies the classes directly.

  .input-field-dark {
    @apply mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm bg-white dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500;
  }
